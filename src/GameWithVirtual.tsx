...
const GameWithVirtual: React.FC<RouteComponentProps> = (props: RouteComponentProps) => {
    const useStyles = makeStyles(() => ({
        container: {
            margin: '0 auto',
            padding: '20px',
            maxWidth: '800px' // 変更: 最大幅を広げて横並びのレイアウトに対応
        },
        inputContent: {
    ...
    const addMove = (move: string) => {
        const solutionTmp = mySolution.concat(move);
        setMySolution(solutionTmp);
        judgeSolution(solutionTmp);
    };
    ...
    const judgeSolution = (mySolution: Array<string>) => {
        ...
        <div>
            ...
            <Box className={classes.container} maxWidth={"xs"} display={"flex"} flexDirection={"column"}>
                ...
                <Box display={"flex"} justifyContent={"center"} alignItems={"center"}>
                    ...
                    <MyButton color='default' width='200px' onClick={() => setOpenScramble(true)}>{t('スクランブル表示')}</MyButton>
                </Box>
                <Box display={"flex"} justifyContent={"center"} alignItems={"center"}>
                    <CubePlayer
                        setupAlg={''}
                        scramble={inverse.inverse(scramble + ' ' + solutionListToString(mySolution)) as string}
